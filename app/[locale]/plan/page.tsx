...
export default async function PlanIndexPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'app' });
  ...
  return (
    <PlanProvider>
      <div className="w-full max-w-md flex flex-col gap-4">
        ...
        <p className="text-muted-foreground">Browse all 365 days of the reading plan.</p>
          ...
          {RAW_PLAN_DATA.map((passages, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">{translatedPassages.join(', ')}</div>
              </Link>
            );
          })}
        </div>
